function [Y, out] = FlagCGcay(fun, Y0, opts, dim, varargin)
% Conjugate gradient method on Flag manifold: Cayley transform
% Input: fun - objective 
%        Y0 - initial point
%        opts - options
...
for k = 1: mxitr
    ...
    if methodretr == 1
        [U,VtU,VtY] = preCay1(Z,Y);
    else
        [YtZ,ZtZ,YYtZ,ZtYYtZ,M1,M2,M3,M4] = preCay2(Z,Y);
    end
    ...
    while count < 5
        if methodretr == 1
            [Yt,M] = RetrCay1(t,U,VtU,VtY,Y);
        else
            [Yt,M5,M6] = RetrCay2(t,Z,YtZ,ZtZ,YYtZ,ZtYYtZ,M1,M2,M3,M4,Y);
        end
    ...
    % vector transport
    if methodretr == 1
        T = VecTran1(t,U,VtU,VtY,M);
    else
        T = VecTran2(t,Y,YtZ,ZtZ,M1,M2,M4,M5,M6);
    end
